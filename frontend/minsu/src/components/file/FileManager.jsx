...
const FileManager = (props) => {
  ...
  const fileDelete = async (postId, fileId) => {
    try {
      const response = await axios.delete(`${import.meta.env.VITE_REST_API}/post/${postId}/file/delete?fileId=${fileId}`, {headers: headers});
        ...
        {files.map((file) => (
          <li key={file.fileId} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <span>
              ...
              {/* 파일 다운로드 버튼 */}
              <a href={`${import.meta.env.VITE_REST_API}/post/${postId}/file/download?fileId=${file.fileId}`} download>
                Download
              </a>
            </span>
            {/* 삭제 버튼을 가장 오른쪽에 배치하기 */}
            <button
              ...
              onClick={() => fileDelete(postId, file.fileId)}
            >
              삭제
            </button>
          </li>
